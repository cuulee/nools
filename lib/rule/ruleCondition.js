...
function isExtra(extra) {
    return extd.isString(extra) && FROM_REGEXP.test(extra);
}
...
function parseParamType(type, scope) {
    ...
    if (extd.isString(type)) {
        return normalizeScope[type] || lookupParamType(type);
    } else if (extd.isFunction(type)) {
        return type;
...
class RuleCondition {
    ...
    constructor(type, alias, constraint, store, from, scope) {
        ...
        this.scope = scope;
        this.parsedConstraint = parser.parseConstraint(this.constraint);
        if (this.from.from) {
            this.parsedFromConstraint = parser.parseConstraint(this.from.from);
        }
    }
    ...
    static fromConditionArray(condition, scope) {
        ...
        if (isExtra(constraint)) {
            ...
            constraint = 'true';
        } else if (extd.isHash(constraint)) {
            from = store;
            store = constraint;
            constraint = 'true';
            if (isExtra(from)) {
