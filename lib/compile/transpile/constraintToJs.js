...
function parseConstraintModifier(constraint) {
    if (constraint.length && _.isString(constraint[0])) {
...
function parseConstraintHash(constraint, identifiers) {
    if (!(constraint.length && _.isPlainObject(constraint[0]))) {
        return '';
    }
    const refs = constraint.shift();
    _(refs).values().forEach((ident) => {
...
function formatConstraint(constraint, identifiers) {
    ...
    if (constraint.length) {
        ...
        if (_.isString(c) && c) {
            ...
            parser.constraint.getIdentifiers(parser.constraint.parseConstraintSource(c))
